...
**Requires** `sandermuller/boost-core ^0.11` — the engine ships the schema discovery, `boost validate` / `boost slots` / `boost paths` commands, doctor extension, the `->withConventions([...])` builder method on `BoostConfig`, and the marker-bounded writer that renders the `## Project Conventions` block on `boost sync`. `0.11.0` adds the `BoostWrapperContract` wrapper-emit-surface so bare-CLI `boost sync` no longer false-positive-flags wrapper-injected files for deletion (the correctness half of the wrong-entry-point bug class; `0.10.0` closed the discoverability half with the `boost doctor` entry-point-mismatch banner). The earlier `0.10.0` cross-agent capability-loss fix + `boost tags` three-case diagnostic split, plus the `0.9.3` data-loss patch + `0.9.4` diagnostic-visibility improvements, ride along transitively. Per the load-bearing-only floor-pin discipline, polish-tier improvements within `0.11.x` will ride the range constraint without forcing the floor higher.
