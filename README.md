...
**Requires** `sandermuller/boost-core ^1.2`. Slot-aware skills carry **paired visible-default tokens** — `<!--boost:conv path="…" mode="…"-->default words<!--boost:conv:end-->` — which `boost-core` resolves at sync time, replacing the whole span with the configured value (or the schema default, or the visible default as fallback). The visible default is what makes the catalog render cleanly under **any** engine: one that doesn't resolve `boost:conv` — notably `laravel/boost`, which installs `SKILL.md` and preserves HTML comments verbatim — leaves the comments inert, so the default words read as ordinary prose instead of a word gap. The paired form needs `boost-core 1.2.0+`; an older engine would emit both the resolved value and the visible default, hence the `^1.2` floor. (Earlier the catalog used bare `<!--boost:conv … fallback="…"-->` tokens — resolver since `0.15.0`, `mcp.*` sub-keys since `0.16.0` — whose fallback lived inside the comment and so vanished under `laravel/boost`; the paired form is the cross-engine fix.)
