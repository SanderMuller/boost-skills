...
**Requires** `sandermuller/boost-core ^0.10` — the engine ships the schema discovery, `boost validate` / `boost slots` / `boost paths` commands, doctor extension, the `->withConventions([...])` builder method on `BoostConfig`, and the marker-bounded writer that renders the `## Project Conventions` block on `boost sync`. The `^0.10` floor is load-bearing: `boost-core 0.10.0` closes a silent cross-agent capability-loss class — Laravel projects wiring `BoostAutoSync::run` (the bare-CLI hook) silently lost bundled `pest-testing` / `livewire-development` / `filament-development` / Inertia / Flux / Volt / Tailwind / Wayfinder / `laravel-best-practices` skills to Cursor / Copilot / Codex (Claude Code masked the gap via laravel/boost's MCP server). `0.10` adds a `boost doctor` entry-point-mismatch banner gated on `project-boost-laravel` presence and splits the `boost tags` "possible typo" diagnostic into three case-specific fix paths. The earlier `0.9.3` data-loss patch + `0.9.4` diagnostic-visibility improvements ride along transitively. Per the load-bearing-only floor-pin discipline, polish-tier improvements within `0.10.x` will ride the range constraint without forcing the floor higher.
