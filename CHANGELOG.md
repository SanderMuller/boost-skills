...
- `resolve-conflicts`: `dangling-symbols.sh` reads a typed class constant (`const int LIMIT`, nullable and union types included) as its name, not its type. Adding a type to a constant no longer reports every use of it, and a renamed typed constant is now caught.
- `dangling-symbols.sh` no longer counts `use function` and `use const` imports as declarations, so removing one no longer reports every call to that function.
- `dangling-symbols.sh` skips Markdown and PHPStan baseline files. Prose no longer shows up as a stale reference, and a baseline message no longer hides a real one. Other `.neon` files are still scanned.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.47.0...2.47.1

## 2.47.0 - 2026-09-24

### Added

- `resolve-conflicts` handles stacked pull requests. It detects a `gh stack` branch, asks whether to update the whole stack or only part of it, and rebases with `gh stack rebase` instead of merging. It checks each rebased branch against the branch below it, keeps `#` lines in commit messages, and confirms on GitHub after the push that no pull request still conflicts.

### Fixed

- `resolve-conflicts` states that ours and theirs are swapped during a rebase, and no longer suggests a merge for a stack.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.46.1...2.47.0

## 2.46.1 - 2026-09-23

### Fixed

- `database-specialist` no longer trusts `->instant()` alone. On a framework version without that modifier the call does nothing, so the subagent now requires the generated SQL to show `algorithm=instant`, or a raw statement that states it.
- `database-specialist` names MySQL 8.0.12 as the first version with instant `ADD COLUMN`, instead of calling the change safe on any MySQL 8.
- `accessibility-reviewer` no longer reports a toggle with a stable name and an updating `aria-pressed` as a failure. It reports a toggle whose state is never exposed, or whose label and pressed state contradict each other.
- `sentry-researcher` lists the Sentry read tools by name instead of a server wildcard, so a write tool the server adds later stays out of reach.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.46.0...2.46.1
...
- The `codex-review` wrapper pins the review model and reasoning effort. It runs Codex with `--ignore-user-config`, so the user's own model and effort never reached a run and every review fell back to the Codex CLI defaults. The wrapper now sends `gpt-5.6-sol` at `medium` effort, with `--model` / `--effort` and `CODEX_REVIEW_MODEL` / `CODEX_REVIEW_EFFORT` as overrides. Under `--use-user-config` it sends neither, so the user config stays authoritative.
- The `pre-release` skill bounds its CI wait and fetches before the gauntlet. An unbounded wait on a commit that matches no workflow path filter never ended, and a branch found to be behind at push time cost a second run of every local check.

### Internal

- The dangling-symbol self-test covers the forced-colour case.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.44.0...2.45.0

## 2.44.0 - 2026-09-17

### Changed

- The `resolve-conflicts` companion script sweeps in one pass per stage. It ran a repo-wide `git grep` per candidate symbol, so a merge that removed hundreds of declarations took minutes; one pass now collects every declared name, and the reference scan runs one alternation per chunk of names.
- The same script fails closed where the old shape could not. A `git grep` exit of 2 or higher is an error rather than "no match", filenames come back NUL-separated so a path holding a colon still parses, and only regex-safe names reach the alternation pattern.
- An import line no longer counts as a declaration, and generated bundles (`*.map`, `*.min.js`) are skipped by both passes. Either one would mark a removed name as still declared and report a clean sweep over a broken tree.

### Internal

- A 17-case self-test covers the script and runs in CI. Every guard in it is invisible until a case exercises it, and the one answer it must never give is "no dangling references" because it could not run.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.43.0...2.44.0

## 2.43.0 - 2026-09-16

### Changed

- `humanizer`: the skill states its scope. It covers prose a person reads — a README, a docs page, a release note, a PR or issue body, a review comment, a commit message.
- `humanizer`: agent-to-agent text is out of scope — a peer message, a subagent prompt, a subagent report, a handoff note. Prose carried inside a tool call, such as a `gh pr create --body` value, stays in scope.
- `humanizer`: the frontmatter description carries the same limit, so another agent matches on it before it opens the file.
- `voice` guideline: messages between agents get Simplified Technical English and never a `humanizer` pass.

### Fixed

- The README inventory summary said 35 skills. The catalogue ships 36.

**Full Changelog**: https://github.com/SanderMuller/boost-skills/compare/2.42.0...2.43.0
...
- `readme`: a README opens from the reader's problem, not the package's feature list. All three shapes now start with one or two sentences that name the situation the reader is in without the package and what the package does about it, then the first code block. The first example resolves that problem instead of touring the API.
- `readme`: the why-rule now separates why the package exists (the opening, kept) from why a feature works the way it does (rationale, cut), so a verbosity audit no longer deletes the opening as motivation prose.
- `readme`: a feature-list description is a new verbosity-audit finding. The `pre-release` audit table lists it too, scoped to the README.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.35.1...2.36.0

## 2.35.1 - 2026-08-31

### Fixed

- `eye-verification`: the skill told an agent to read `references/eye-verify.md`, `scripts/README.md` and the Playwright harness without saying where they live. It ships none of those files — they belong to `frontend-quality` — so the paths resolved against the wrong skill directory and found nothing. The skill now names the base directory once and links the three documents at `../frontend-quality/…`, the sibling-link form the rest of the catalog already uses.

**Full Changelog**: https://github.com/SanderMuller/boost-skills/compare/2.35.0...2.35.1

## 2.35.0 - 2026-08-31

### Added

- `humanizer`: an agentic-era hype-word list. "battle-tested", "table stakes" and "north star" are cut for the fact they stand in for.
- `humanizer`: a context-checked list beside it — `byte-identical`, `load-bearing`, "first-class citizen", "non-trivial" and "step change" are real technical terms as often as they are hype, so each is flagged only where the word adds importance the fact does not carry.
- `humanizer`: section 39, for documentation that argues for its own design where the reader wanted an instruction, and for the virtue words a tool uses about itself. Rationale still stands where it names a trade-off or explains a constraint that would otherwise read as a defect.
- `humanizer`: a Quick Checks bullet for each new pattern.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.34.0...2.35.0

## 2.34.0 - 2026-08-30

### Added

- `codex-review`: the wrapper reports `wrapperVersion`, so a consumer can date a stale copy.
- `readme`: rules for the agent-facing surface — `llms.txt`, `llms-full.txt`, per-page markdown, `robots.txt`.
- `readme`: docs sites open with a Getting started page carrying one full scenario.
- `release-notes`: name the observable a regression would change, in place of a test count.

### Changed

- `codex-review`: a SIGKILL with no output and no `wrapperVersion` now routes to update-and-re-sync, not to `code-review`.

### Internal

- The release workflow strips the `verified-sha` pin from the release body before it reaches `CHANGELOG.md`.
- Cut the README prose to the three tables CI enforces.

**Full Changelog**: https://github.com/SanderMuller/boost-skills/compare/2.33.0...2.34.0
...
- `readme`: coverage tiers judged from the repository — critical path, public API, or self-describing.
- `readme`: a critical tier that overrides every budget. A security caveat, a data-loss or irreversibility warning, a required prerequisite, an accessibility note, or a compatibility limit is documented in full and in place — never collapsed, never demoted for being rare, never cut to hit a number.
- `readme`: a verbosity audit beside the staleness audit, and a `<details>` rule for the long tail.
- `release-notes`: a bullets-only body with a 15-word average budget, a pre-publish gate, and a security-fix shape.

### Changed

- `readme`: paragraphs cap at ~100 words, counted between blank lines, not lines.
- `readme`: the curse-of-knowledge rule now applies to the critical path only.
- `release-notes`: five fixed sections, no intro prose, no `###` headings.
- `release-notes`: breaking migrations, security impact, data-loss and accessibility warnings, and required prerequisites are exempt from the word budget and excluded from its average. They stay inside a bullet, and the pre-publish gate carries the same exemption.
- `release-notes`: a security bullet names attacker capability, exposure, and affected versions, so a `CHANGELOG.md` reader can judge urgency without opening the advisory.
- `pre-release`: the verbosity audit runs over every current README and docs page, not the diff range.
- `pre-release`: its inline release-notes fallback lists `Changed`, which it previously dropped.

**Full changelog:** https://github.com/SanderMuller/boost-skills/compare/2.32.2...2.33.0
...
- `quality.pipeline` convention. A project that declares several pipelines can name the one `evaluate` asks about, and `evaluate` appends `--pipeline=<value>`. Absent, `evaluate` asks without it — a multi-pipeline project then gets an error naming its pipelines and runs its checks normally rather than guessing which one to trust.

**Full Changelog**: https://github.com/SanderMuller/boost-skills/compare/2.30.0...2.31.0
...
- The README now points at the published documentation site instead of repository markdown, and keeps the skill inventory itself.

### Fixed

- `codex-review` now sends the review prompt to Codex on stdin instead of as a command-line argument. Every focused review failed before this: the `codex` binary is killed by the OS at exec time when any single argument is larger than about 1010 bytes, so it died with no output and nothing to report. The limit is per argument rather than per command line, and a short prompt stays under it — which is why the failure only showed up on real reviews.

**Full Changelog**: https://github.com/SanderMuller/boost-skills/compare/2.29.0...2.30.0
