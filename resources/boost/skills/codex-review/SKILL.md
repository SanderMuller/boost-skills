...
- Because the user's Codex config is ignored, the wrapper pins the reasoning effort itself: `medium` by default. Override per-run with `--effort <low|medium|high|xhigh|max|default>`, or set `CODEX_REVIEW_EFFORT`; `default` leaves the effort to the Codex CLI. Raise it to `high` only for a review that needs it — effort drives token usage on every loop iteration.
- The model is pinned to `gpt-6-sol` for the same reason. It needs a recent Codex CLI: an older CLI (0.153.4, for example) rejects it with "The 'gpt-6-sol' model is not supported when using Codex with a ChatGPT account". On that error, update the CLI (`npm install -g @openai/codex`) or pass `--model gpt-5.6-sol`. Override with `--model <model>` or `CODEX_REVIEW_MODEL`; the flag wins over the env var. With `--use-user-config` and no override, the model and the effort both come from the user config.
...
- **`ok: false` with `signal: "SIGKILL"`, `resultBytes: 0`, an empty `stderrPreview`, and no `wrapperVersion` field** — Codex was killed before it produced any output. The missing field dates the report to a wrapper older than the one this skill ships with. Such a wrapper sends the review prompt on argv. The binary is killed at exec time when a single argument is too large, so every focused review dies this way. Update the package, re-sync, then run the review again. This is the one `ok: false` that a re-run can clear — do not fall back to `code-review` before the re-sync. A report that carries `wrapperVersion` already sends the prompt on stdin, so read that failure under the next bullet.
...
**One attempt, then fall back — do not loop the wrapper.** It cannot hang, so a failure is a genuine failure (auth, capacity, a real Codex error), not a transient wedge that a re-run clears. The stale-wrapper SIGKILL above is the one exception, and only after a re-sync.
...
- **A hand-rolled `codex exec` sends the prompt on stdin** — if you ever invoke `codex exec` directly instead of through the wrapper, redirect the prompt from a file (`codex exec … < prompt.md`). Never pass it as an argument: the binary is killed at exec time when a single argument is too large, which is the failure the wrapper exists to avoid.
