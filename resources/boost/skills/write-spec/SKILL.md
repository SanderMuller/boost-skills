...
> For a feature that renders UI, the spec's success measure includes **eye-verify** — the change is best confirmed by *seeing it run* in a real browser (per the `frontend-quality` skill), not by type-check/lint alone; the `## Edge Cases` rows then double as the browser testables.
