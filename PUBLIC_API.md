...
The semver-protected surface of `sandermuller/boost-skills`. Anything documented here is covered by semver; anything outside is internal and may change without notice (including in patch releases).
...
- `Sandermuller\BoostSkills\YourPublicClass` — purpose
-->
...
Anything under `Sandermuller\BoostSkills\Internal`, anything marked `@internal` in PHPDoc, anything in `src/Support/` and `src/Concerns/`.
